  ...
  const shake = async () => {
    ...
    try {
      ...
      if (ethereum) {
        ...
        const shakeTxn = await wavePortalContract.shake(userMessage, {
          gasLimit: 300000,
        });
  ...
  return (
    <div className="mainContainer">
      <div className="dataContainer">
        <div className="header">👋 Hey there, shaker!</div>

        <div className="bio">
          I'm Casey - a Frontend Engineer and Community Manager at DEV. As we
          head into fall, we're going to be shaking some trees. Connect your
          Ethereum wallet and make the leaves fall!
        </div>
        ...
        <div className="message-container">
          {allShakes.reverse().map((shake, index) => {
            return (
              <div key={index} className="message slideup">
                ...
                <div className="info">
                  <p>{shake.address}</p>
                  <p>
                    {shake.timestamp.toLocaleDateString()} -{" "}
                    {shake.timestamp.toLocaleTimeString()}
                  </p>
                </div>
              </div>
            );
